...
class WarningFilter {
    static shouldIgnore(message: unknown): boolean {
        ...
        return IGNORED_WARNING_MESSAGES.some((warningMsg) => message.includes(warningMsg));
    }
}
...
export const setupGlobalErrorHandling = (
    ...
    const errorHandler = new ErrorHandler(customErrorHandler, finalConfig.maxErrorsPerSecond);

    // 设置 window.onerror 处理器
    window.onerror = (message, source, lineno, colno, error) => {
        errorHandler.handle(error || new Error(message as string), `window.onerror: ${source}:${lineno}:${colno}`);
        return true;
    };
